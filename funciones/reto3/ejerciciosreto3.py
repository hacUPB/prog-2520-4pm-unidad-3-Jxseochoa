...
def ejercicio3():
    ...
    for nombre_fase, tiempo_fase, consumo_fase in [
        ("ascenso", t_ascenso, consumo_ascenso),
        ("crucero", t_crucero, consumo_crucero),
        ("descenso", t_descenso, consumo_descenso)
    ]:
        for minuto in range(0, tiempo_fase, intervalo):
            ...
            if combustible_act <= 0:
                print("Minuto", tiempo_total, ": Sin combustible durante", nombre_fase, ". Vuelo fallido.")
                estado_final = "Vuelo fallido"
                break
            elif combustible_act <= min_seguridad:
                print("Minuto", tiempo_total, ": Combustible", combustible_act, "litros → Emergencia en", nombre_fase)
            else:
                print("Minuto", tiempo_total, ": Combustible", combustible_act, "litros → Normal en", nombre_fase)

        if estado_final == "Vuelo fallido":
            break

    print("---- FIN DEL VUELO ----")
    print("Tiempo total:", tiempo_total, "minutos")
    print("Combustible final:", combustible_act, "litros")
